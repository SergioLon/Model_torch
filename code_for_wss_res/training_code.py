...
def training(hyperParams,model,data_loaders,optimizer,scheduler,mesh_path):
    ...
    nmae_metr=np.zeros((2,hyperParams["epochs"]),dtype=np.dtype('float32'))
    ...
    for epoch in range(hyperParams['epochs']):
        try:
            ...
            f_nmse.write(str(train_loss.detach().numpy())+"    "+ str(val_loss.detach().numpy())+"\n")
            f_nmae.write(str( train_metric.detach().numpy())+"    "+str(val_metric.detach().numpy())+"\n")
            ...
            break
    return model,saved_loss,nmae_metr,cosine_simil,mre_saved
