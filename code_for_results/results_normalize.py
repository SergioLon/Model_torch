...
def predict_on_dataloader(mesh_path,model,data_loaders):
    ...
    for idx,m in enumerate(data_loaders['val']):
            ...
            #ax.title('One Val sample')
            ax.set_xlabel('Vertx')
            ax.set_ylabel('MESH '+str(idx)+' WSS_ABS normalized')
            plt.show()
            plt.savefig(mesh_path+'/MESH_' +str(idx)+'_abs.png')
            ...
            wss_abs_p=denormalize_std_1_wss(out.cpu().detach().numpy(),m.wss_std_abs.cpu(),m.wss_mean_abs.cpu())
            mesh.point_arrays["wss_abs_pred"]=wss_abs_p
            ...
            wss_abs=denormalize_std_1_wss(m.wss_abs.cpu().detach().numpy(),m.wss_std_abs.cpu(),m.wss_mean_abs.cpu())
            mesh.point_arrays["wss_abs"]=wss_abs
            ...
            #ax.title('One Val sample')
            ax.set_xlabel('Vertx')
            ax.set_ylabel('MESH' +str(idx)+' |WSS_ABS-WSS_ABS_PRE|')
            plt.show()
            plt.savefig(mesh_path+'/MESH_' +str(idx)+'_dabs.png')
            #ABS
            # fig, ax = plt.subplots()
            ...
            #ax.title('One Val sample')
            ax.set_xlabel('Vertx')
            ax.set_ylabel('MESH' +str(idx)+'Error WSS_ABS')
            plt.show()
            plt.savefig(mesh_path+'/MESH_' +str(idx)+'_eabs.png')
            # #ABS
            # fig, ax = plt.subplots()
            # err_abs=np.abs((mesh.point_arrays["wss_abs_pred"]-mesh.point_arrays["wss_abs"]))/max(abs(mesh.point_arrays["wss_abs"]))
            # ax.plot(err_abs)
            ...
            # print(len(err_y))
            # print(len(err_z))
            mesh.point_arrays["err"]=np.expand_dims(err_abs,-1)
            print("Mean Error ABS: ",np.mean(err_abs))
            f.write("Mean Error ABS: %f \n" %np.mean(err_abs))
            ...
            r_err_abs=relative_error(mesh.point_arrays["wss_abs_pred"],mesh.point_arrays["wss_abs"])
            #print("err_x.size(0)")
            ax.plot(r_err_abs)
            #ax.legend()
            #ax.title('One Val sample')
            ax.set_xlabel('Vertx')
            ax.set_ylabel('MESH' +str(idx)+' Relative Error WSS_ABS')
            plt.show()
            plt.savefig(mesh_path+'/MESH_' +str(idx)+'_reabs.png')
            # #ABS
            # fig, ax = plt.subplots()
            # err_abs=np.abs((mesh.point_arrays["wss_abs_pred"]-mesh.point_arrays["wss_abs"]))/max(abs(mesh.point_arrays["wss_abs"]))
            # ax.plot(err_abs)
            ...
            mesh.point_arrays["r_err"]=np.expand_dims(r_err_abs,-1)
            print("Mean Relative Error ABS: ",np.sum(r_err_abs)/mesh.n_points)
            f.write("Mean Relative Error ABS: %f \n" %(np.sum(r_err_abs)/mesh.n_points))
